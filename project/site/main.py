...
async def create_request_endpoint(data: UserBuildRequest):
    """Создание запроса от пользователя"""
    request_doc = {
        **data.dict(),
...
def get_current_admin(credentials: HTTPAuthorizationCredentials = Depends(security)):
    """Проверка токена и роли админа"""
    token = credentials.credentials
    if not token or not token.startswith('Bearer '):
        raise HTTPException(status_code=401, detail="Неверный токен")
    
    token = token.replace('Bearer ', '')
    # Простая проверка (замените на вашу логику валидации)
    user_data = users_collection.find_one({"auth_token": token})
...
async def admin_update_component(
    ...
    success = mongo.update_component(component_type, component_id, update_data)
    if not success:
...
def get_current_developer(credentials: HTTPAuthorizationCredentials = Depends(security)):
    """Проверка токена и роли разработчика"""
    token = credentials.credentials
    if not token or not token.startswith('Bearer '):
        raise HTTPException(status_code=401, detail="Неверный токен")
